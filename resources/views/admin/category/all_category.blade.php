...
@section('admin_content')
    ...
    <div class="side-app">
        <div class="title" style="display: flex; align-items: center; justify-content: space-between">
            ...
            <a style="font-size: 14px; display: block" href="{{ URL::to('/admin/category/add') }}"
        ...
        <table class="table" style="text-align: center">
            <thead class="bg-default">
              <tr>
                ...
                <th scope="col">Ngày chình sửa gần nhất</th>
            ...
            @foreach($all_category as $key => $val)
                <tr>
                    ...
                    <td style="display: flex; justify-content: center">{{-- {{$val->status}} --}}
                        <img src="/upload/products/{{$val->image}}" width="150px" height="150px" alt="">
                    </td>
                    <td>{{date('d-m-Y', strtotime($val->created_at))}}</td>
                    <td>{{date('d-m-Y', strtotime($val->updated_at))}}</td>
                    <td style="font-size: 20px">
                        <a href="{{URL::to('/admin/category/edit/'.$val->id)}}"
                        ...
                        <a onclick="return confirm('Bạn có chắc là muốn xóa danh mục này không?')"
                            ...
                            href="{{URL::to('/admin/category/delete/'.$val->id)}}"
